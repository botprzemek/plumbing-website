...
<template>
  ...
  <nav v-if="!mobile" class="sticky top-0 z-30 px-phone sm:px-fit flex flex-row justify-center sm:justify-between h-16 bg-white shadow-sm">
    ...
    <ul class="hidden sm:flex flex-row gap-4 items-center">
      <li>
        <button v-if="path === '/'" @click="scrollTo('header')" class="text-blue text-base udr-anim">
          <span>Start</span>
        </button>
        <RouterLink v-else to="/" class="text-black text-base hover:text-blue transition-colors duration-200 ease-in udr-anim">
          <span>Start</span>
        </RouterLink>
      </li>
      <li v-for="route in routes()" :key="route">
        ...
        <RouterLink v-else :to="route.link" class="text-black text-base hover:text-blue transition-colors duration-200 ease-in udr-anim">
          <span>{{ route.name }}</span>
        </RouterLink>
      </li>
      <li>
